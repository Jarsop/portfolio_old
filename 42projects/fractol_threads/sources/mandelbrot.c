/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   mandelbrot.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jgeslin <jgeslin@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/05/28 16:33:09 by jgeslin           #+#    #+#             */
/*   Updated: 2016/09/17 17:32:37 by jgeslin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/fractol.h"

void	ft_help_mand(t_mlx *mlx, int *i)
{
	double	tmp;

	tmp = 0;
	while (mlx->z_r * mlx->z_r + mlx->z_i * mlx->z_i < 4 &&
			*i < mlx->iteration_max)
	{
		tmp = mlx->z_r;
		mlx->z_r = mlx->z_r * mlx->z_r - mlx->z_i * mlx->z_i + mlx->c_r;
		mlx->z_i = 2 * mlx->z_i * tmp + mlx->c_i;
		++*i;
	}
}

void	ft_draw_mandelbrot(t_mlx *mlx)
{
	int		x;
	int		y;
	int		i;

	x = -1;
	while (++x < WIN_X)
	{
		y = -1;
		while (++y < WIN_Y)
		{
			i = 0;
			mlx->c_r = x / mlx->zoom + mlx->x1 + mlx->x_o;
			mlx->c_i = y / mlx->zoom + mlx->y1 + mlx->y_o;
			mlx->z_r = 0;
			mlx->z_i = 0;
			ft_help_mand(mlx, &i);
			if (i != mlx->iteration_max)
				mlx->imgc[x + y * WIN_X] = i * 189979;
		}
	}
}

int		ft_loop_mandelbrot(t_mlx *mlx)
{
	mlx_destroy_image(mlx->mlx, mlx->img);
	mlx->img = mlx_new_image(mlx->mlx, WIN_X, WIN_Y);
	ft_draw_mandelbrot(mlx);
	mlx_put_image_to_window(mlx->mlx, mlx->win, mlx->img, 0, 0);
	return (0);
}

void	ft_mandelbrot(void)
{
	t_mlx	mlx;

	ft_init_mandelbrot(&mlx);
	mlx.mlx = mlx_init();
	mlx.win = mlx_new_window(mlx.mlx, WIN_X, WIN_Y, "Mandelbrot");
	mlx.img = mlx_new_image(mlx.mlx, WIN_X, WIN_Y);
	mlx.imgc = (int*)mlx_get_data_addr(mlx.img, &mlx.bpp, &mlx.sl, &mlx.e);
	mlx_hook(mlx.win, 2, (1L << 0), ft_key_mandelbrot, &mlx);
	mlx_mouse_hook(mlx.win, ft_mouse_hook, &mlx);
	mlx_loop_hook(mlx.mlx, ft_loop_mandelbrot, &mlx);
	mlx_loop(mlx.mlx);
}
